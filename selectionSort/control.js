...
class controler{
  ...
  */
  sort(){
    ...
    for(let i=0;i<n;i++){
      let minIdx=i;
      setTimeout(()=>{arr.select(i);},TERM*cnt);
      cnt++;
      for(let j=i+1;j<n;j++){
        setTimeout(()=>{arr.select(j)},TERM*cnt);
        cnt++;
        if(arr.at(minIdx)>arr.at(j)){
          let tmp=minIdx;
          setTimeout(()=>{arr.select(tmp,'none')},TERM*cnt);
          minIdx=j;
          cnt++;
        }
        else{
      ...
      cnt++;
      if(i!=minIdx){
        arr.swapNum(i,minIdx);
        setTimeout(()=>{arr.swap(i,minIdx,TERM/2);},TERM*cnt);
        cnt++;
      }
      setTimeout(()=>{arr.select(minIdx,'none')},TERM*cnt);
